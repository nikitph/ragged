...
Usage:
    ...
    # Run benchmarks
    python wikipedia_search.py --benchmark
...
class WikipediaSearchEngine:
    ...
    def run_benchmarks(self) -> Dict[str, Any]:
        ...
        print("\n🧪 Running Wikipedia Search Benchmarks...")
        print("=" * 60)
        ...
        # Resource Benchmarks
        print("\n💻 Resource Benchmarks")
        results['resources'] = self._benchmark_resources()
        ...
        filename = f"wikipedia_benchmark_{timestamp}.json"
    ...
    def _benchmark_performance(self) -> Dict[str, float]:
        ...
        test_queries = [
            "machine learning", "quantum physics", "artificial intelligence",
            "neural networks", "computer science", "programming languages",
            "data structures", "operating systems"
        ]

        # Cold start performance
        print("   ❄️ Cold start latency...")
        self.decoder.memory_cache.clear()

        cold_times = []
        for query in test_queries[:3]:
            start_time = time.time()
            results = self.search(query, top_k=5, show_performance=False)
            cold_times.append(time.time() - start_time)

        # Warm performance
        print("   🔥 Warm search latency...")
        warm_times = []
        ...
        for query in test_queries:
            start_time = time.time()
            if self.use_model_server:
                vector, _ = self.client.encode(query)
            else:
                vector = self.model.encode([query])[0]
            encoding_times.append(time.time() - start_time)

        return {
            "cold_start_p50_ms": statistics.median(cold_times) * 1000,
            "cold_start_p95_ms": np.percentile(cold_times, 95) * 1000,
            "warm_search_p50_ms": statistics.median(warm_times) * 1000,
            "warm_search_p95_ms": np.percentile(warm_times, 95) * 1000,
            "encoding_p50_ms": statistics.median(encoding_times) * 1000,
            "encoding_p95_ms": np.percentile(encoding_times, 95) * 1000,
            "throughput_qps": len(test_queries) / sum(warm_times)
        }

    def _benchmark_quality(self) -> Dict[str, float]:
        """Benchmark search quality"""
        test_cases = [
            {
                "query": "anarchist principles schools thought",
                "expected_topics": ["anarchist", "their", "consensus"],
                "min_similarity": 0.6
            },
            {
                "query": "state domination authority power",
                "expected_topics": ["state", "that", "anarchist"],
                "min_similarity": 0.6
            },
            {
                "query": "violence terrorism propaganda deed",
                "expected_topics": ["violence", "early", "state"],
                "min_similarity": 0.5
            },
            {
                "query": "anarchist tactics revolutionary evolutionary",
                "expected_topics": ["during", "anarchists", "with"],
                "min_similarity": 0.6
            },
            {
                "query": "consensus decision making groups",
                "expected_topics": ["consensus", "other", "their"],
                "min_similarity": 0.7
            },
            {
                "query": "contemporary anarchism small groups",
                "expected_topics": ["invisible", "anarchists", "spaces"],
                "min_similarity": 0.5
            }
        ]

        print("   📊 Relevance and coverage...")
        ...
        for test_case in test_cases:
            results = self.search(test_case["query"], top_k=10, show_performance=False)
            ...
            # Relevance
            relevant_count = sum(1 for r in results if r['similarity'] >= test_case["min_similarity"])
            relevance_scores.append(relevant_count / len(results))
            ...
            coverage_scores.append(len(found_topics) / len(test_case["expected_topics"]))
        ...
        return {
            ...
            "coverage_score": statistics.mean(coverage_scores),
            "diversity_score": statistics.mean(diversity_scores),
            "avg_similarity": statistics.mean(similarities) if similarities else 0.0
        }

    def _benchmark_resources(self) -> Dict[str, float]:
        ...
        test_queries = ["machine learning", "quantum physics", "AI ethics", "neural networks"]

        for query in test_queries * 3:  # 12 total queries
        ...
        # Cache stats
        cache_hits = len(self.decoder.memory_cache)
        total_fragments = len(self.decoder.manifest["metadata"]["fragments"])
        cache_hit_rate = cache_hits / total_fragments if total_fragments > 0 else 0
        ...
        return {
            ...
            "memory_growth_mb": max(memory_measurements) - initial_memory,
            "cache_hit_rate": cache_hit_rate,
            "cached_fragments": cache_hits
        }
    ...
    def _print_benchmark_summary(self, results: Dict):
        ...
        # Calculate grades
        avg_latency = perf["warm_search_p50_ms"]
        if avg_latency < 100:
            perf_grade = "A"
        ...
        print(f"⚡ Performance Grade: {perf_grade} ({avg_latency:.1f}ms avg)")
        print(f"🎯 Quality Grade: {quality_grade} ({relevance:.1%} relevance)")
        print(f"🚀 Throughput: {perf['throughput_qps']:.1f} queries/sec")
        print(f"💾 Cache Hit Rate: {resources['cache_hit_rate']:.1%}")

        print(f"\n📈 Detailed Metrics:")
        print(f"   Cold Start p95: {perf['cold_start_p95_ms']:.0f}ms")
        print(f"   Warm Search p95: {perf['warm_search_p95_ms']:.0f}ms")
        print(f"   Query Encoding: {perf['encoding_p50_ms']:.1f}ms")
        print(f"   Result Diversity: {quality['diversity_score']:.1%}")
        print(f"   Memory Usage: {resources['peak_memory_mb']:.1f}MB")

        # Recommendations
        print(f"\n💡 Recommendations:")
        if perf['warm_search_p50_ms'] > 500:
            print("   • Consider optimizing fragment caching")
        if quality['relevance_score'] < 0.8:
            print("   • Review chunking strategy and embedding model")
        if resources['cache_hit_rate'] < 0.5:
            print("   • Increase cache size or improve prefetching")
        if resources['memory_growth_mb'] > 500:
...
Default R2 URLs:
  ...
  Faiss: {DEFAULT_FAISS_URL}
    ...
    parser.add_argument("--benchmark", action="store_true",
                        help="Run comprehensive benchmarks and exit")
    ...
    try:
        ...
        print("  pip install sentence-transformers faiss-cpu numpy requests psutil")
